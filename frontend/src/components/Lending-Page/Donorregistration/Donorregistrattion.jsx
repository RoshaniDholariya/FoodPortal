...
//     try {
//       const response = await axios.post("http://localhost:3000/api/donors/add-details", formDataToSend, {
...
//   const isFormValid = () => {
//     const requiredFields = ["name", "address", "city", "state", "pincode", "phone", "donorType"];
//     const hasRequiredFields = requiredFields.every((field) => formData[field].trim() !== "");
//     const isRestaurantValid =
//       formData.donorType !== "RESTAURANT" || (formData.donorType === "RESTAURANT" && formData.restaurantName.trim() !== "");
...
//                 onChange={handleInputChange}
//                 className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#8beb7f] focus:border-transparent h-24"
...
const DonorRegistrationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = {
      donorId: parseInt(donorId), // Make sure donorId is included and parsed
      name: formData.name,
      ...
      restaurantName: formData.donorType === "RESTAURANT" ? formData.restaurantName : null,
    };
  ...
  // Update the convertToBase64 function to handle the image size
  const convertToBase64 = (file) => {
    return new Promise((resolve, reject) => {
      ...
      fileReader.onload = () => {
        // Remove the data:image/[type];base64, prefix if needed by your backend
